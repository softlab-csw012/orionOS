...
static bool parse_mount_source_drive(const char* src, int* out_drive) {
    if (!src || !out_drive) {
        return false;
    }
...
static bool dispatch_mount(const char *orig_cmd, char *cmd, bool *out_success) {
    ...
    if (src[0] == '\0' || target[0] == '\0' || *sp != '\0') {
        kprint("Usage: mount -l | mount <drive#|/dev/node> <target>\n");
        *out_success = false;
        return true;
    }

    int drive = -1;
    if (!parse_mount_source_drive(src, &drive)) {
