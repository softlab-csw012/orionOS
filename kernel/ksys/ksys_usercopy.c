...
int ksys_copy_user_argv(uint32_t argv_ptr, int argc, int max_argc, uint32_t max_str_len, char*** out_argv) {
    if (!out_argv) {
        return -1;
    }
    *out_argv = NULL;
    ...
    if (ksys_validate_user_buffer(argv_ptr, bytes) != 0) {
        return -1;
    }
    ...
    for (int i = 0; i < argc; i++) {
        ...
        if (ksys_copy_user_string(buf, user_str, max_str_len) != 0) {
            kfree(buf);
            ksys_free_kernel_argv(argv, argc);
            return -1;
        }
