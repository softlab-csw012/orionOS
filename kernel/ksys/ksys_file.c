...
bool ksys_handle_file(registers_t* regs) {
    ...
    switch (eax) {
        ...
        case SYS_OPEN_EX: {
            ...
            sys_open_ex_t req = *(sys_open_ex_t*)ebx;
            if (!req.path_ptr) {
                regs->eax = (uint32_t)-1;
                return true;
            }
        ...
        case SYS_MKNOD: {
            ...
            sys_mknod_t req = *(sys_mknod_t*)ebx;
            if (!req.path_ptr) {
                regs->eax = 0;
                return true;
            }
        ...
        case SYS_READ: {
            ...
            int r = vfs_read(owner_pid, ebx, (void*)regs->edx, ecx);
            if (r < 0) {
        ...
        case SYS_WRITE: {
            ...
            int r = vfs_write(owner_pid, ebx, (const void*)regs->edx, ecx);
        ...
        case SYS_DIR_LIST: {
            ...
            int count = vfs_list_dir(use_path, (char*)req.names_ptr,
