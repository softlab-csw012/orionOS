...
bool devfs_resolve_subpath(const char* subpath, char* out_path, size_t out_len) {
    ...
    if (!subpath || subpath[0] == '\0' || (subpath[0] == '/' && subpath[1] == '\0')) {
        if (out_len < 5) {
            return false;
        }
        strcpy(out_path, "/dev");
    ...
    int n = snprintf(out_path, (int)out_len, "/dev/%s", name);
    return n > 0 && (size_t)n < out_len;
}

bool devfs_exists_subpath(const char* subpath) {
    char path[DEVFS_PATH_MAX];
    if (!devfs_resolve_subpath(subpath, path, sizeof(path))) {
        return false;
    }
    if (strcmp(path, "/dev") == 0) {
        return true;
    }
    return devfs_find_path(path) >= 0;
}
...
int devfs_list_subpath(const char* subpath, char* names, uint8_t* is_dir, uint32_t max_entries, size_t name_len) {
    ...
    return devfs_list("/dev", names, is_dir, max_entries, name_len);
}
