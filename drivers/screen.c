...
static bool g_vga_text_enabled = true;
...
void screen_disable_vga_text(void) {
    g_vga_text_enabled = false;
}

bool screen_vga_text_enabled(void) {
    return g_vga_text_enabled;
}
...
void screen_scroll_output(uint8_t attr) {
    hydrate_buffer_from_vga_once();
    int old_start = 0;
    if (g_fb.enabled) {
    ...
    if (!g_fb.enabled && g_vga_text_enabled) {
...
void screen_refresh_font_metrics(void) {
    if (!g_fb.enabled)
        return;
    ...
    uint8_t font_w = font_get_width();
    uint8_t font_h = font_get_height();
    int cols = font_w ? (int)(g_fb.width / font_w) : MAX_COLS;
    int rows = font_h ? (int)(g_fb.height / font_h) : MAX_ROWS;
