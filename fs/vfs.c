...
static bool vfs_path_is_dev(const char* path) {
    return path && strncmp(path, "/dev/", 5) == 0 && path[5] != '\0';
}
...
int vfs_open_ex(uint32_t owner_pid, const char* path, uint32_t flags) {
    ...
    dev_type_t dev_type = dev_type_from_path(path);
    bool dev_exists = devfs_lookup(path, NULL, NULL, NULL);
    if (dev_type != DEV_NONE || dev_exists) {
        ...
        return fd_attach_device_for_pid(owner_pid, path);
    }

    if (vfs_path_is_dev(path)) {
        return -1;
    }
...
bool vfs_mknod(const char* path, uint8_t node_type, uint16_t major, uint16_t minor) {
    ...
    return devfs_mknod(path, node_type, major, minor);
}

int vfs_list_dir(const char* path, char* names, uint8_t* is_dir, uint32_t max_entries, size_t name_len) {
    if (path != NULL && path[0] != '\0' &&
        (strcasecmp(path, "/dev") == 0 || strcasecmp(path, "/dev/") == 0)) {
        int n = devfs_list(path, names, is_dir, max_entries, name_len);
        if (n >= 0) {
            return n;
        }
    }
